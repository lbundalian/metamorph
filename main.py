...
def main():
    ...
    try:
        ...
        # print(f"   - models/: Schema classes + parsers/ PASSED")
        # print(f"   - morphers/: KDKMorpher conversion PASSED")
        # print(f"   - utils/: Validation methods PASSED")
        print(f"API validation: {'VALID' if is_valid else 'INVALID'}")
        print(f"Your exact API: KDK(json) -> KDKMorpher(kdk_obj, 'RD') PASSED")
        
        # show architecture details
        # print(f"\nSupported target schemas: {morpher.get_supported_schemas()}")
        # print(f"Clean architecture pattern: {morpher}")
